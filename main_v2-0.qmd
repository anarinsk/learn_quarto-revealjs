...
format: 
    revealjs:
        ...
        #preview-links: true
        #self-contained: true
        #Default size: 1050 x 700 (width * 2/3)
        width: 100%
        height: 100%
---
...
:::{.nonincremental}
- 다음 슬라이드에 소개될 `@import`를 활용하는 방법은 폰트 로딩에 과도하게 많은 트래픽을 사용한다. 
- 이 방법은 더 이상 권장하지 않는다.
- 두 가지 해법을 활용한다. 
  - 사용할 폰트에 Variable 폰트가 있으면, 이를 활용한다. 
  - `@font-face`를 활용해서 웹 폰트로 특화된 `*.woff2` 파일을 로딩한다.
- [🔗`format.scss`](https://github.com/anarinsk/learn_quarto-revealjs/blob/main/format.scss)를 참고하자.  
:::

## With `scss` (Depreciated) { .smaller }
...
`[🔗LINK](...){preview-link="true"}` 와 같이 `{}` 안에서 개별적으로 설정해서 쓸 수도 있다. 
...
- 아래의 옵션을 넣어 [렌더링된 문서](./main_v2-0_larger.html)와 비교해보시라. 
- 가능하면 %로 제어하도록 하자. 
:::
...
format: 
    revealjs:
        ...
        width: 100%
        height: 100%  # maybe 3:2 ratio 
```

# 끝 { 
  background-iframe="https://www.youtube.com/embed/yTpQPs2GUVA?autoplay=1&mute=0&start=10" 
