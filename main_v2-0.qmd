...
## 꽉찬 벽지 I {background-color="black" background-image="https://www.comicbasics.com/wp-content/uploads/2020/01/Top-10-Marvel-Superheroes-That-Have-Doctorates-678x381.jpg" background-size="cover" background-opacity="0.5"}

- `background-size="cover"`: 벽지를 슬라이드에 맞춘다. 그림은 임의로 확대된다. 

## 꽉찬 벽지 II {background-color="black" background-image="https://www.comicbasics.com/wp-content/uploads/2020/01/Top-10-Marvel-Superheroes-That-Have-Doctorates-678x381.jpg" background-size="contain" background-opacity="0.5"}
...
목차(왼쪽 아래 삼선)에 제목을 나오게 하려면 다음과 같이 처리하자. 

```qmd
## {data-menu-title="타이틀 없는 슬라이드" background-color="aquamarine"}
```

## {data-menu-title="또 다른 타이틀 없는 슬라이드"   
...
- 이제 가상 환경을 잘 지원한다! 
   ...
   - 페이지 렌더링이 필요하다면 VS Code 안에서 F1으로 인터 프리터를 설정한 뒤 VS Code 내 터미널에서 `>quarto render`를 통해 프로젝트를 컴파일하자. 
...
## 백그라운드 동영상 재생 II{ background-iframe="https://www.youtube.com/embed/yFE6qQ3ySXE?autoplay=1&mute=0&loop=1&playlist=yFE6qQ3ySXE" background-opacity=0.35 }
...
## { data-menu-title="GitHub Pages (cont'd)" }
...
# End of Document { background-iframe="https://www.youtube.com/embed/rvc5mRATQ7c?autoplay=1&start=10&mute=1&loop=1&playlist=rvc5mRATQ7c" background-opacity=0.2 }
