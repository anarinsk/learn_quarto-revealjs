...
- ~~Code Extension이 아직까지 환경을 제대로 지원하지 않는다.~~
  - 이제 잘 지원한다. Conda env로 들어가서 VS Code에서 동일하게 사용하면 된다. 
- 이 슬라이드처럼 다크 테마를 쓸 경우 그림 표시에 문제가 있을 수 있다. 
  - 대안은 그림 표현 역시 다크 테마에 맞추거나 
  - 그림을 표현할 때는 밝은 테마를 쓰는 것이다. 



## Testing code: 'dark_background'
...
- 옵션 해설 
  - `#| output-location: slide`: 별도의 슬라이드에 결과물 배치 
  - `plt.style.use('seaborn-white')`: 스타일 설정  
